      ...
      >
        ...
        <main className={pageStyles.main} style={{ 
          ...
          <article className={blogPostStyles.blogPostWrapper} style={{ 
            ...
            {idea.image && (
              <div className={blogPostStyles.bannerContainer} style={{ 
                  width: '100%', 
                  height: isFullScreen ? '300px' : '450px', 
                  overflow: 'hidden', 
                  border: '1px solid var(--outline-color)',
                  margin: isFullScreen ? '2rem 0' : undefined,
                  display: isFullScreen ? 'block' : (idea.image ? 'block' : 'none')
              }}>
              ...
              {mode === 'reader' ? (
                <div className={`blog-content ${styles.readerContent}`} style={{ border: 'none', background: 'transparent', maxWidth: isFullScreen ? '100%' : undefined }}>
                  {idea.sections && idea.sections.map((section: any, index: number) => {
                    ...
                    return (
                      <section key={section.id || index} id={sectionSlug} className={styles.blogSection} style={{ scrollMarginTop: '100px', marginBottom: '4rem' }}>
                          ...
                          {!isFullScreen && (
                            <div className={styles.sectionStatusBar}>
                              <div className={styles.statusItem} style={{ position: 'relative' }}>
                                ...
                                <span className={styles.statusValue}>{section.paragraphs.join(' ').trim().split(/\s+/).filter(Boolean).length}</span>
                                <div className={styles.verticalTooltip}>Word Count</div>
                              </div>
                              <div className={styles.statusItem} style={{ position: 'relative' }}>
                                <MessageSquare size={16} className={styles.statusIcon} />
                                <span className={styles.statusValue}>{section.paragraphs.join('').length}</span>
                                <div className={styles.verticalTooltip}>Character Count</div>
                              </div>
